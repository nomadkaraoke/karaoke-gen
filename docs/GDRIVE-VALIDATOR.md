...
- **2026-09-11**: NOMAD-1632 **MP4-720p-only gap** (Nat King Cole – **Portrait** of Jennie, job `21150e73`). Unlike the recycled-brand-code gaps above, only the **720p** variant was missing — MP4 and CDG were present and correct — so recycling the brand code would have been *wrong* (it would have duplicated the existing MP4/CDG). **Root cause: a filename-substring collision with the song title.** The encoder produced and uploaded the 720p to GCS correctly (`… (Final Karaoke Lossy 720p).mp4`), but `GCEEncodingBackend.encode()` mapped the worker's returned file list to format keys by matching the **whole filename** — checking `"portrait" in filename_lower` (to detect the 9:16 portrait video) *before* `"720p"`. The title "Portrait of Jennie" contains "portrait", so the 720p master matched the portrait branch and was filed as `portrait_mp4`; `mp4_720p` was never set. The orchestrator then had `final_video_720p=None`, and `gdrive_service.upload_to_public_share` **silently skips** a file whose path is `None` → the track published to MP4/ + CDG/ with no MP4-720p/ entry. Deterministic for **any** song whose artist/title contains a format word ("portrait", "720p", "4k", "lossless", "vocals", …). The daily validator was the only detector (~a day later). Fixes (v0.222.2): (1) a shared, title-collision-proof `classify_encoded_output()` matches the **format parenthetical only** (never the whole filename), used by both the encode path and `redistribute_video`; (2) a **completeness guard** in `GCEEncodingBackend.encode()` — if the worker's returned `output_files` is missing any requested `format`, it returns `success=False` (orchestrator raises), so any *other* partial-encode cause also fails loud and is retried instead of publishing an incomplete release. **Remediation of the live gap:** the correct 720p already existed in GCS (`jobs/21150e73/finals/…(Final Karaoke Lossy 720p).mp4`), so it was uploaded to MP4-720p/ + the Divebar mirror directly — no re-render, no YouTube disruption. Do NOT recycle 1632.
